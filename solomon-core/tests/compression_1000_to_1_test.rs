...
use solomon_zk::verify_stark_proof;
...
fn test_compression_1000_to_1_end_to_end() {
    ...
    // 2.1 CRITICAL: The generated proof MUST be independently verified, not just sized.
    // This is what separates a real STARK compression claim from a demo gag.
    let verify_start = std::time::Instant::now();
    let verified = verify_stark_proof(&stark_proof, sample_pk, sample_payload)
        .expect("verifier errored on a proof generated by the prover");
    let verify_duration = verify_start.elapsed();
    assert!(verified, "Generated STARK proof must verify against its public key");
    ...
    println!("  • STARK Prove Latency:              {:.2} ms", prove_duration.as_secs_f64() * 1000.0);
    println!("  • STARK Verify Latency:             {:.2} ms", verify_duration.as_secs_f64() * 1000.0);
    println!("  • PROVE->VERIFY ROUNDTRIP:          {} (must be true)", verified);
    println!("  -----------------------------------------------------");
